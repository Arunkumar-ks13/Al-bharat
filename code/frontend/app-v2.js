...
function processSituation() {
    const situationOriginal = document.getElementById('situation-input').value.trim();
    const situation = situationOriginal.toLowerCase();
    ...
    // Keywords for farmer welfare (check both original and lowercase)
    ...
    console.log('Input:', situationOriginal, 'Lowercase:', situation);
    console.log('isEducation:', isEducation, 'isFarmer:', isFarmer);
...
// Show farmer schemes with AI recommendation
async function showFarmerSchemes() {
    showLoading(true);
    
    try {
        // Get AI-powered scheme recommendations for farmers
        const userInput = document.getElementById('situation-input').value.trim();
        
        const response = await fetch(`${API_BASE_URL}/process-input`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                userInput: `Farmer welfare schemes: ${userInput}`,
        ...
        // AI analyzes and recommends schemes
        document.getElementById('eligibility-message').innerHTML = 
            '<strong style="color: #48bb78;">AI-Recommended Farmer Welfare Schemes</strong><br>' +
            '<em style="color: #667eea;">Based on your needs, here are the best schemes for you</em>';
        ...
        showLoading(false);
        showToast(error.message || 'Error loading schemes', 'error');
...
// Generate farmer application form with AI
async function generateFarmerForm() {
    if (!selectedScheme) {
    ...
    try {
        // Use AI to generate personalized farmer form
        const response = await fetch(`${API_BASE_URL}/generate-form`, {
            ...
            body: JSON.stringify({
                ...
                userDetails: {
                    ...
                    userInput: document.getElementById('situation-input').value
                }
            })
        });
        ...
        // Add farmer-specific fields if not present (no education fields needed)
        if (!formData.fullName) formData.fullName = '[Your Full Name]';
        if (!formData.fatherName) formData.fatherName = '[Father\'s Name]';
        if (!formData.dateOfBirth) formData.dateOfBirth = '[Date of Birth (DD/MM/YYYY)]';
        if (!formData.gender) formData.gender = '[Gender]';
        if (!formData.aadhaarNumber) formData.aadhaarNumber = '[12-digit Aadhaar Number]';
        if (!formData.mobileNumber) formData.mobileNumber = '[10-digit Mobile Number]';
        if (!formData.address) formData.address = '[Complete Address with Village/Town]';
        if (!formData.district) formData.district = '[District Name]';
        if (!formData.taluk) formData.taluk = '[Taluk Name]';
        if (!formData.village) formData.village = '[Village Name]';
        if (!formData.surveyNumber) formData.surveyNumber = '[Land Survey Number]';
        if (!formData.landArea) formData.landArea = '[Total Land Area in Acres]';
        if (!formData.cropType) formData.cropType = '[Type of Crop Cultivated]';
        if (!formData.bankAccountNumber) formData.bankAccountNumber = '[Bank Account Number]';
        if (!formData.ifscCode) formData.ifscCode = '[IFSC Code]';
        if (!formData.bankName) formData.bankName = '[Bank Name]';
        if (!formData.branchName) formData.branchName = '[Branch Name]';
        if (!formData.reasonForApplication) formData.reasonForApplication = '[Reason for Application]';
        ...
        showToast('AI-generated farmer application form ready!');
        
        // Setup intelligent validation
        setTimeout(() => {
...
// Submit farmer application with AI backend
async function submitFarmerApplication() {
    showLoading(true);
    ...
    try {
        ...
        const downloadUrl = `${data.downloadUrl}?${cacheBuster}`;
        
        // Update download link
        const downloadLink = document.getElementById('download-link');
        if (downloadLink) {
            downloadLink.href = downloadUrl;
            downloadLink.setAttribute('download', `AGAA-Farmer-Application-${data.applicationId}.html`);
        }
        ...
        const submissionSection = document.querySelector('.submission-instructions');
        ...
        // Update Next Steps section for farmer context
        const nextStepsSection = document.querySelector('.next-steps-section');
        nextStepsSection.innerHTML = `
            <h3>⏱️ Next Steps & Support</h3>
            <div class="next-steps-box">
                <div class="next-step-item">
                    <span class="next-step-icon">📅</span>
                    <div class="next-step-content">
                        <strong>Processing Time</strong>
                        <p>Your application will be processed within 6 weeks from submission</p>
                    </div>
                </div>
                <div class="next-step-item">
                    <span class="next-step-icon">📞</span>
        ...
        showToast('Farmer application submitted successfully!');
...
async function generateForm() {
    ...
    try {
        ...
        // Setup intelligent validation after a short delay
        setTimeout(() => {
            setupIntelligentValidation();
        }, 500);
...
// Intelligent field validation with AI
let validationTimeout = null;
const validationCache = new Map();

async function validateFieldWithAI(fieldName, fieldValue) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/validate-field`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                fieldName,
                fieldValue,
                formContext: {
                    educationLevel: eligibilityAnswers.educationLevel,
                    studyField: eligibilityAnswers.studyField,
                    annualIncome: eligibilityAnswers.annualIncome,
...
function showValidationMessage(inputElement, validation) {
    ...
    // Don't show message if no content
    if (!validation.message && !validation.suggestion) return;
    
    // Create validation message element
    const msgDiv = document.createElement('div');
    msgDiv.className = `validation-message validation-${validation.severity}`;
    ...
    let content = `<span class="validation-icon">${icons[validation.severity] || '💡'}</span>`;
    content += `<span class="validation-text">${validation.message}</span>`;
    
    if (validation.suggestion) {
        content += `<br><span class="validation-suggestion">Suggestion: ${validation.suggestion}</span>`;
    }
    
    if (validation.additionalInfo) {
        content += `<br><span class="validation-info">${validation.additionalInfo}</span>`;
    }
    ...
    // Auto-apply suggestion if it's a success/info and user hasn't modified much
    if (validation.suggestion && (validation.severity === 'success' || validation.severity === 'info')) {
        const applyBtn = document.createElement('button');
        applyBtn.className = 'btn-apply-suggestion';
        applyBtn.textContent = 'Apply';
        applyBtn.onclick = () => {
            inputElement.value = validation.suggestion;
            msgDiv.remove();
...
function setupIntelligentValidation() {
    console.log('Setting up intelligent validation...');
    // Add validation to all form inputs
    const formInputs = document.querySelectorAll('#form-fields input, #form-fields textarea, #form-fields select');
    ...
    formInputs.forEach(input => {
        input.addEventListener('blur', async function() {
            ...
            console.log('Blur validation for', fieldName, ':', fieldValue);
            
            // Show loading indicator
            this.classList.add('validating');
            
            // Validate with AI
            const validation = await validateFieldWithAI(fieldName, fieldValue);
        ...
        input.addEventListener('input', function() {
            ...
            validationTimeout = setTimeout(async () => {
                const fieldName = this.id.replace('field-', '');
                const fieldValue = this.value;
                console.log('Input validation for', fieldName, ':', fieldValue);
                
                if (fieldValue.length > 3) { // Only validate if meaningful input
                    this.classList.add('validating');
                    const validation = await validateFieldWithAI(fieldName, fieldValue);
                    this.classList.remove('validating');
                    console.log('Validation result:', validation);
    ...
    console.log('Intelligent validation setup complete');
}
...
async function submitApplication() {
    ...
    try {
        ...
        // Add cache-busting with timestamp AND random string to ensure uniqueness
        const cacheBuster = `t=${Date.now()}&r=${Math.random().toString(36).substr(2, 9)}`;
        const downloadUrl = `${data.downloadUrl}?${cacheBuster}`;
        
        console.log('Setting download URL:', downloadUrl); // Debug log
        
        // Update download link
        const downloadLink = document.getElementById('download-link');
        if (downloadLink) {
            downloadLink.href = downloadUrl;
            downloadLink.setAttribute('download', `AGAA-Application-${data.applicationId}.html`);
            console.log('Download link updated successfully'); // Debug log
        } else {
            console.error('Download link element not found!'); // Debug log
        }
...
function startOver() {
    ...
    // Reset confirmation page to default (scholarship context)
    ...
    // Reset submission instructions to default (scholarship)
    document.querySelector('.submission-instructions').innerHTML = `
        <h3>📋 How to Submit Your Application</h3>
        <div class="instruction-box">
            <p><strong>Option 1: Online Submission</strong></p>
            <p>Visit the Tamil Nadu e-Governance Agency portal:</p>
            <a href="https://ssp24-25.tnega.org/institute/approve-list.html" target="_blank" class="portal-link">
                https://ssp24-25.tnega.org/institute/approve-list.html
            </a>
            <p>Upload your filled application form and required documents.</p>
        </div>
        ...
        <div class="instruction-box">
            <p><strong>Option 2: Offline Submission</strong></p>
            <p>1. Download and print your application form</p>
            <p>2. Attach required documents (Aadhaar, income certificate, etc.)</p>
            <p>3. Visit your local District Education Office</p>
            <p>4. Submit the printed application with documents</p>
        </div>
    ...
    // Reset Next Steps section to default (scholarship)
    document.querySelector('.next-steps-section').innerHTML = `
        <h3>⏱️ Next Steps & Support</h3>
        <div class="next-steps-box">
            <div class="next-step-item">
                <span class="next-step-icon">📅</span>
